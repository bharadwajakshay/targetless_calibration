...
main_path = args.path
save_path_prefix = '/'.join(main_path.split('/')[:-3])
save_path_prefix = os.path.join(save_path_prefix,'processed')
save_path_suffix = '/'.join(main_path.split('/')[-2:])
savepath = os.path.join(save_path_prefix, save_path_suffix)
...
if not os.path.exists(savepath + "_sync/depth_maps"):
    os.makedirs(savepath + "_sync/depth_maps")

if not os.path.exists(savepath + "_sync/intensity_maps"):
    os.makedirs(savepath + "_sync/intensity_maps")

if not os.path.exists(savepath + "_sync/target_imgs"):
    os.makedirs(savepath + "_sync/target_imgs")

if not os.path.exists(savepath + "_sync/depth_maps_transformed"):
    os.makedirs(savepath + "_sync/depth_maps_transformed")

if not os.path.exists(savepath + "_sync/intensity_maps_transformed"):
    os.makedirs(savepath + "_sync/intensity_maps_transformed")

depth_maps_folder = savepath + "_sync/depth_maps"
intensity_maps_folder = savepath + "_sync/intensity_maps"
target_img_folder = savepath + "_sync/target_imgs"
depth_maps_transformed_folder = savepath + "_sync/depth_maps_transformed"
intensity_maps_transformed_folder = savepath + "_sync/intensity_maps_transformed"
...
for img_name, cloud_name in zip(imgs_files, point_files):
    ...
    # Get the no of points in the point cloud
    if maxPtCldSize < points.shape[0]:
        maxPtCldSize = points.shape[0]

    intensityValues = points[:,3]
    points = points[:,:3]
    ones_col = np.ones(shape=(points.shape[0],1))
    points = np.hstack((points,ones_col))

    # current_img = smc.imread(img_name)
    current_img = imageio.imread(img_name)
    ...
    xidx = np.where((x >= 0) & (x <= img_wdt - 1))
    yidx = np.where((y>=0) & (y <= img_ht - 1))
    intensityIdx = np.intersect1d(xidx,yidx)

    xint = x[intensityIdx]
    yint = y[intensityIdx]
    zint = Z[intensityIdx]
    intensityValues_transformed = intensityValues[intensityIdx]
    ...
    imageio.imwrite(depth_maps_transformed_folder + "/" + img_name[-14:], reprojected_img)

    reprojected_img = np.zeros_like(img)
    ittrCount = 0
    for x_idx, y_idx,z_idx,int_idx in zip(xint,yint,zint,intensityValues_transformed):
        if(z_idx>0):
            reprojected_img[int(y_idx), int(x_idx)] = int_idx*255
            ittrCount+=1

    imageio.imwrite(intensity_maps_transformed_folder + "/" + img_name[-14:], reprojected_img)
    ...
    yint = y[intensityIdx]
    zint = Z[intensityIdx]
    intensityValues = intensityValues[intensityIdx]
    ...
    imageio.imwrite(depth_maps_folder + "/" + img_name[-14:], pooled_img)
    imageio.imwrite(target_img_folder + "/" + img_name[-14:], reconstructed_img)

    reprojected_img = np.zeros_like(img)
    ittrCount = 0
    for x_idx, y_idx,z_idx,int_idx in zip(xint,yint,zint,intensityValues):
        if(z_idx>0):
            reprojected_img[int(y_idx), int(x_idx)] = int_idx*255
            ittrCount+=1

    imageio.imwrite(intensity_maps_folder + "/" + img_name[-14:], reprojected_img)



np.savetxt(depth_maps_transformed_folder + "/../angle_list.txt", angle_list[1:], fmt = "%.4f")
f=open(depth_maps_transformed_folder + "/../maxPtCldSize.txt",'w')
f.write(str(maxPtCldSize))
f.close()